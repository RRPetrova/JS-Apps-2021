function attachEvents() {
    ...
    function loadPhonebook() {
        clearOutput();
        fetch(`http://localhost:3030/jsonstore/phonebook.json`)
            .then(res => res.json())
            .then(data => {
                Object.entries(data)
    ...
    function createPhonebook() {
        ...
        fetch(`http://localhost:3030/jsonstore/phonebook.json`, headers)
            .then(() => {
                clearOutput();
                personInput.value = "";
                phoneInput.value = "";
    ...
    function deletePhonebook() {
        ...
        fetch(`http://localhost:3030/jsonstore/phonebook${phonebookId}.json`, headers)
            .then(() => {
                clearOutput();
                loadPhonebook();
            })
